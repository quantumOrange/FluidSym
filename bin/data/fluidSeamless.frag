...
float fracNoise(vec4 P, int octaves, float lacunarity, float gain)
{
    float sum = 0.0;
    float amp = 1.0;
    vec4  pp = P;
...
void main()
{
   ...
   // if(abs(advection.x) < threashold ) advection.x = 0.0;
   // if(abs(advection.x) < threashold ) advection.y = 0.0;
    vec4 density =  texture(tex0,texCoordVarying + advection*inverseSize);
    
    //vec4 density =  texture(tex0, texCoordVarying );
